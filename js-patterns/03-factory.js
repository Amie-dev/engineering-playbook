/**
 * ============================================================
 *  FILE 3 : Corner Cafe Stall — Factory Pattern
 *  Topic  : Simple Factory, Factory with Registration
 *  Where you'll see this: React.createElement, Express route handlers
 * ============================================================
 */

// STORY: Ramesh operates a popular beverage stall.
// Customers just say what drink they want — he handles the creation logic.

console.log("=== FILE 03: Corner Cafe Stall ===\n");
...
function makeDrink(type, name) {
  const base = { name: name, madeBy: "Master Brewer" };
  ...
  if (type === "masala") {
    ...
    base.strength = 9;
    base.price = 20;
    base.vessel = "earthenware";
  } else {
    throw new Error("Cannot make unknown beverage: " + type);
  }

  return base;
}

const masala = makeDrink("masala", "Spiced Special");
const cutting = makeDrink("cutting", "Quick Cup");

console.log("Masala:", masala.name, "| strength:", masala.strength);
console.log("Cutting:", cutting.name, "| price: ₹" + cutting.price);

try {
  makeDrink("bubble", "Mystery");
} catch (err) {
...
class BeverageRegistry {
  ...
  register(type, creatorFn) {
    ...
    console.log("Master Brewer learned to brew: " + type);
  }
...
registry.register("herbal", function (name) {
  return { name: name, type: "herbal", strength: 6, price: 25 };
});

const myMasala = registry.create("masala", "Kadak Spiced");
const myHerbal = registry.create("herbal", "Kerala Herbal");

console.log("Created:", myMasala.name, "₹" + myMasala.price);
console.log("Created:", myHerbal.name, "₹" + myHerbal.price);
...
console.log("\n=== Master Brewer cleans the workstation. Another productive day. ===");
