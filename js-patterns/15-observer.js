...
// STORY: Govind is the Street Vendor. He announces "Orders ready!"
...
let govind = new StreetVendor("Govind");
...
govind.notify("Spiced tea ready!");

govind.unsubscribe(autoDriver);
govind.notify("Fresh coffee ready!");
...
let vendorBus = new EventEmitter();

vendorBus.on("order:ready", function (type) {
  console.log("Shopkeeper: " + type);
});

vendorBus.once("special:offer", function (msg) {
  console.log("Watchman (once): " + msg);
});

vendorBus.emit("order:ready", "Ginger tea brewing");
vendorBus.emit("special:offer", "Snacks at half price!");
vendorBus.emit("special:offer", "Another offer!");
